...
function removeOrphaned($block, $closeQuotePos) {
    // After the closing quote of a value, remove everything up to the next field
    $rest = substr($block, $closeQuotePos + 1);
    if (preg_match("/(\n\s+'[a-z_]+'\s*=>)/s", $rest, $m, PREG_OFFSET_CAPTURE)) {
        $nextFieldFull = $m[1][0];
        $nextFieldPos = $m[1][1];
        // Return: block up to closing quote + comma + newline + next field + everything after
        return substr($block, 0, $closeQuotePos + 1) . ",\n" . ltrim($nextFieldFull, "\n, ") . substr($rest, $nextFieldPos + strlen($nextFieldFull));
    }
    // If no next field, check for end of block
    $eob = strpos($rest, '],');
    if ($eob !== false) {
        // end of block
        $orphaned = substr($rest, 0, $eob);
        return substr($block, 0, $closeQuotePos + 1) . ",\n" . substr($rest, $eob);
    }
    return $block;
}
...
foreach ($blocks as &$block) {
    ...
    // Replace short_description value if it's a string (not null)
    ...
    if ($sdPos !== false) {
        ...
        if ($sdEnd !== false) {
            ...
            // Cleanup orphaned text after short_description
            $block = removeOrphaned($block, $sdEnd);
        }
    }
    ...
    if ($dPos !== false) {
        ...
        if ($dEnd !== false) {
            ...
            // Cleanup orphaned text after description
            $block = removeOrphaned($block, $dEnd);
        }
    }
...
echo "Products: " . preg_match_all("/'price'\s*=>\s*\d+/", $v) . "\n";

// Check for orphaned text patterns
$bad = ["display it.'s", "oil.</p>'s", "collector's"];
$found = false;
foreach ($bad as $b) {
    if (str_contains($v, $b)) { echo "BAD: $b\n"; $found = true; }
}
if (!$found) echo "No orphaned text patterns found.\n";
