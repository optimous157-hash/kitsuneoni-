...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-yamagata-silver text-sm mt-0.5">Welcome back, {{ auth()->user()->name }}</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <a href="{{ route('admin.orders.index') }}" class="admin-stat group">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-xs text-yamagata-silver uppercase tracking-wide">Total Orders</p>
                    <p class="text-2xl font-bold text-white mt-1.5">{{ number_format($stats['total_orders']) }}</p>
                </div>
            ...
            <div class="flex items-center gap-1 mt-2.5">
                ...
                <span class="text-xs text-yellow-400">{{ $stats['pending_orders'] }} pending</span>
            </div>
            @endif
        </a>
        ...
        <div class="admin-stat">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-white mt-1.5">${{ number_format($stats['total_revenue'], 0) }}</p>
                </div>
        ...
        <a href="{{ route('admin.products.index') }}" class="admin-stat group">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-xs text-yamagata-silver uppercase tracking-wide">Products</p>
                    <p class="text-2xl font-bold text-white mt-1.5">{{ number_format($stats['total_products']) }}</p>
                </div>
        ...
        <a href="{{ route('admin.newsletter.index') }}" class="admin-stat group">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-xs text-yamagata-silver uppercase tracking-wide">Subscribers</p>
                    <p class="text-2xl font-bold text-white mt-1.5">{{ number_format($stats['newsletter_subscribers']) }}</p>
                </div>
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            <h2>Order Status</h2>
            <a href="{{ route('admin.orders.index') }}" class="text-xs text-yamagata-red hover:text-yamagata-red-light transition-colors">View all →</a>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
            @foreach(['pending' => 'yellow', 'confirmed' => 'blue', 'processing' => 'purple', 'delivered' => 'green', 'cancelled' => 'red'] as $status => $color)
            <a href="{{ route('admin.orders.index', ['status' => $status]) }}" class="relative p-4 rounded-xl bg-yamagata-charcoal/30 border border-yamagata-graphite/30 hover:border-{{ $color }}-500/30 transition-all text-center group">
                <p class="text-2xl font-bold text-white">{{ $ordersByStatus[$status] ?? 0 }}</p>
                <p class="text-xs text-yamagata-silver mt-1 capitalize">{{ $status }}</p>
                <span class="absolute top-3 right-3 w-2 h-2 bg-{{ $color }}-400 rounded-full opacity-60"></span>
            </a>
            @endforeach
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <p class="text-xs text-yamagata-steel">{{ $order->customer_email }}</p>
                            </td>
                            <td class="text-right">
                                <span class="text-white font-semibold text-sm">${{ number_format($order->total, 0) }}</span>
                            </td>
                            <td>
                                <span class="admin-badge admin-badge-{{ $order->status }}">{{ $order->status_label }}</span>
                            </td>
                            <td class="text-right text-yamagata-silver text-xs">{{ $order->created_at->format('M d, H:i') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="space-y-6">
            ...
            <div class="admin-card">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-3 p-2.5 rounded-lg hover:bg-yamagata-charcoal/30 transition-colors">
                        <span class="text-xs font-bold text-yamagata-steel w-4 text-center">{{ $index + 1 }}</span>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm text-white truncate">{{ $product->name }}</p>
                            <p class="text-[11px] text-yamagata-steel">{{ $product->category->name ?? 'Uncategorized' }}</p>
                        </div>
                        <span class="text-xs text-yamagata-silver font-medium">{{ $product->sales_count }}</span>
                    </div>
                    @endforeach
                </div>
                @else
            ...
            <div class="admin-card">
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.products.create') }}" class="flex items-center gap-3 p-3 rounded-xl bg-yamagata-red/5 border border-yamagata-red/10 hover:border-yamagata-red/25 hover:bg-yamagata-red/10 transition-all group">
                    ...
                    <a href="{{ route('admin.orders.index', ['status' => 'pending']) }}" class="flex items-center gap-3 p-3 rounded-xl bg-yamagata-charcoal/30 border border-yamagata-graphite/30 hover:border-yamagata-graphite/50 transition-all group">
                    ...
                    <a href="{{ route('admin.orders.export') }}" class="flex items-center gap-3 p-3 rounded-xl bg-yamagata-charcoal/30 border border-yamagata-graphite/30 hover:border-yamagata-graphite/50 transition-all group">
