...
<div class="space-y-6" x-data="{ showNew: false }">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-yamagata-silver text-sm mt-0.5">{{ number_format($faqs->count()) }} questions</p>
        </div>
        ...
        <div class="overflow-x-auto">
            ...
            <table class="admin-table">
                ...
                <tbody>
                    @foreach($faqs as $faq)
                    <tr>
                        <td class="text-yamagata-steel text-xs">{{ $faq->sort_order }}</td>
                        <td>
                            <span class="text-white text-sm font-medium">{{ $faq->question }}</span>
                        </td>
                        <td class="text-sm max-w-[350px] truncate">{{ $faq->answer }}</td>
                        <td class="text-right">
                            <form action="{{ route('admin.content.faqs.destroy', $faq) }}" method="POST" class="inline" onsubmit="return confirm('Delete this FAQ?')">
                                @csrf @method('DELETE')
