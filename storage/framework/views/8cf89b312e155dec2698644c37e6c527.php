<?php $__env->startSection('title', $product->meta_title ?: $product->name . ' — Kitsuneoni'); ?>
<?php $__env->startSection('description', $product->meta_description ?: $product->short_description); ?>
<?php $__env->startSection('og_title', $product->meta_title ?: $product->name); ?>
<?php $__env->startSection('og_description', $product->meta_description ?: $product->short_description); ?>
...
<?php echo json_encode($productLd, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT); ?>
...
<div class="pt-20 min-h-screen bg-background">
    ...
    <div class="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12 py-4 sm:py-6">
        <nav class="flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-muted-foreground">
            <a href="<?php echo e(route('home')); ?>" class="hover:text-foreground transition-colors">Home</a>
            ...
            <a href="<?php echo e(route('shop.index')); ?>" class="hover:text-foreground transition-colors">Collection</a>
            <?php if($product->category): ?>
            <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg>
            <a href="<?php echo e(route('shop.index', ['category' => $product->category->slug])); ?>" class="hover:text-foreground transition-colors"><?php echo e($product->category->name); ?></a>
            <?php endif; ?>
            <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg>
            <span class="text-foreground max-w-[200px] truncate"><?php echo e($product->name); ?></span>
        </nav>
    </div>
    ...
    <div class="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12 pb-28 lg:pb-24">
        <div class="grid lg:grid-cols-2 gap-8 lg:gap-16">
            ...
            <div class="min-w-0" x-data="{
                ...
                imgLoading: false,
                imgError: false,
                images: <?php echo \Illuminate\Support\Js::from($galleryImages)->toHtml() ?>,
                ...
                openLightbox() {
                    this.lightbox = true;
                    this.imgError = false;
                    const src = this.images[this.activeImage] || <?php echo \Illuminate\Support\Js::from($product->primary_image_url)->toHtml() ?>;
                ...
                <div class="relative w-full sm:aspect-square bg-card overflow-hidden cursor-pointer group rounded-2xl border border-border max-sm:h-[55vw]" @click="openLightbox()">
                    <img
                        :src="images[activeImage] || <?php echo \Illuminate\Support\Js::from($product->primary_image_url)->toHtml() ?>"
                    ...
                    <?php if($product->is_new || $product->is_bestseller || $product->discount_percent): ?>
                    <div class="absolute top-4 left-4 flex gap-2 z-10">
                        ...
                        <span class="px-3 py-1 bg-green-500/20 backdrop-blur-md text-green-600 dark:text-green-400 text-[11px] font-semibold tracking-wider uppercase rounded-full">-<?php echo e($product->discount_percent); ?>%</span>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="flex gap-2.5 mt-4 overflow-x-auto pb-1">
                    ...
                    <button @click="activeImage = <?php echo e($index); ?>" :class="activeImage === <?php echo e($index); ?> ? 'border-primary' : 'border-transparent opacity-60 hover:opacity-100'" class="flex-shrink-0 w-14 sm:w-20 h-14 sm:h-20 overflow-hidden border-2 transition-all rounded-xl">
                        <img src="<?php echo e($img->url); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-cover" loading="lazy" x-on:error="handleThumbError($el)">
                    </button>
                ...
                <div x-show="lightbox" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-[0.97]" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-200" class="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4" @click="lightbox = false">
                    ...
                    <img x-show="!imgError" :src="images[activeImage] || <?php echo \Illuminate\Support\Js::from($product->primary_image_url)->toHtml() ?>" @click.stop
            ...
            <div class="min-w-0 lg:sticky lg:top-28 h-fit">
                ...
                <?php if($product->is_new || $product->is_bestseller): ?>
                ...
                <h1 class="font-heading text-2xl sm:text-4xl lg:text-5xl font-light leading-tight text-foreground mb-4"><?php echo e($product->name); ?></h1>

                
                <?php if($product->short_description): ?>
                <p class="text-muted-foreground leading-relaxed mb-4 sm:mb-8"><?php echo e($product->short_description); ?></p>
                <?php endif; ?>
                ...
                <div class="border border-border p-4 sm:p-6 mb-4 sm:mb-8 rounded-xl">
                    ...
                    <p class="font-mono text-2xl sm:text-3xl text-primary">$<?php echo e(number_format($product->price, 0)); ?> <span class="text-sm text-muted-foreground font-sans">USD</span></p>
                </div>

                
                <?php if(count($specPills) > 0): ?>
                <div class="flex flex-wrap gap-2 mb-4">
                    <?php $__currentLoopData = $specPills; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-muted border border-border rounded-lg text-[11px]">
                        <span class="text-muted-foreground"><?php echo e($sp['label']); ?></span>
                        <span class="text-foreground font-medium"><?php echo e($sp['value']); ?></span>
                    </span>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
                <?php endif; ?>
                ...
                <div class="flex items-center gap-2.5 mb-4">
                    <div class="flex gap-0.5">
                        <?php for($i = 1; $i <= 5; $i++): ?>
                        <svg class="w-3.5 h-3.5 <?php echo e($i <= $product->average_rating ? 'text-yamagata-gold' : 'text-muted-foreground/30'); ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.538 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                        <?php endfor; ?>
                    </div>
                    <span class="text-xs text-muted-foreground"><?php echo e($product->average_rating); ?> (<?php echo e($product->reviews_count); ?> <?php echo e(Str::plural('review', $product->reviews_count)); ?>)</span>
                </div>
                <?php endif; ?>
                ...
                <div class="flex gap-3 mb-4">
                    <a href="<?php echo e(route('order.create', ['product_id' => $product->id])); ?>"
                    ...
                    <button @click="$store.wishlist.toggle({ id: <?php echo e($product->id); ?>, name: '<?php echo e(addslashes($product->name)); ?>', slug: '<?php echo e($product->slug); ?>', price: '$<?php echo e(number_format($product->price, 0)); ?>', image: '<?php echo e($product->primary_image_url); ?>', url: '<?php echo e($product->url); ?>' })"
                            :class="$store.wishlist.has(<?php echo e($product->id); ?>) ? 'border-primary text-primary' : 'border-border text-muted-foreground hover:border-foreground hover:text-foreground'"
                            class="w-14 h-14 border rounded-xl flex items-center justify-center transition-all shrink-0"
                            aria-label="Toggle wishlist">
                        <svg class="w-5 h-5" :class="$store.wishlist.has(<?php echo e($product->id); ?>) ? 'fill-primary' : ''" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                    </button>
                    <button onclick="navigator.share ? navigator.share({title:'<?php echo e(addslashes($product->name)); ?>',url:window.location.href}) : navigator.clipboard.writeText(window.location.href)"
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-xs text-muted-foreground mb-4 sm:mb-8">
                    <div class="flex items-center gap-2">
                        ...
                        <?php if($product->in_stock): ?>
                        <span><?php echo e($product->stock); ?> in stock</span>
                        <?php else: ?>
                        <span>Made to order</span>
                        <?php endif; ?>
                    </div>
                    <?php if($product->sku): ?>
                    <span class="font-mono">SKU: <?php echo e($product->sku); ?></span>
                    <?php endif; ?>
                </div>

                
                <div class="grid grid-cols-3 gap-2 sm:gap-4 pt-6 border-t border-border">
                    <a href="<?php echo e(route('faq')); ?>" class="text-center group">
            ...
            <div x-show="specsOpen" x-collapse>
                <div class="border border-border rounded-xl overflow-x-auto mt-4 sm:mt-6 md:mt-8">
                    <table class="w-full min-w-0">
                        <tbody>
                            <?php $__currentLoopData = $allSpecs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => [$label, $value]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="<?php echo e($index < count($allSpecs) - 1 ? 'border-b border-border' : ''); ?>">
                                <td class="py-3 sm:py-4 px-3 sm:px-6 text-[10px] tracking-[0.2em] uppercase text-muted-foreground w-2/5 sm:w-1/3 align-top break-words"><?php echo e($label); ?></td>
                                <td class="py-3 sm:py-4 px-3 sm:px-6 font-mono text-sm text-foreground break-words"><?php echo e($value); ?></td>
                            </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </tbody>
                    </table>
            ...
            <button @click="reviewsOpen = !reviewsOpen" type="button" class="flex items-start justify-between w-full text-left group md:cursor-default mb-4 md:mb-8">
                <div class="flex items-center gap-4">
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground mt-1"><?php echo e($product->reviews_count); ?> <?php echo e(Str::plural('review', $product->reviews_count)); ?></p>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-xl sm:text-2xl font-bold text-foreground"><?php echo e($product->average_rating); ?></span>
                        <div class="flex gap-0.5">
                            <?php for($i = 1; $i <= 5; $i++): ?>
                            <svg class="w-4 h-4 <?php echo e($i <= $product->average_rating ? 'text-yamagata-gold' : 'text-muted-foreground/30'); ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.538 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                            <?php endfor; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-card border border-border rounded-xl p-4 sm:p-6">
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex gap-0.5 mb-2">
                                    <?php for($i = 1; $i <= 5; $i++): ?>
                                    <svg class="w-3 h-3 <?php echo e($i <= $review->rating ? 'text-yamagata-gold' : 'text-muted-foreground/20'); ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.538 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                    <?php endfor; ?>
                                </div>
                                <?php if($review->title): ?>
                                <h4 class="text-sm font-semibold text-foreground mb-1"><?php echo e($review->title); ?></h4>
                                <?php endif; ?>
                                <p class="text-sm text-muted-foreground leading-relaxed"><?php echo e($review->body); ?></p>
                            </div>
                        ...
                        <div class="flex flex-wrap items-center gap-x-2 gap-y-1 mt-4 pt-3 border-t border-border">
                            <span class="text-xs font-medium text-foreground"><?php echo e($review->customer_name); ?></span>
                            <?php if($review->customer_country): ?>
                            <span class="text-[11px] text-muted-foreground">· <?php echo e($review->customer_country); ?></span>
                            <?php endif; ?>
                            <span class="text-[11px] text-muted-foreground">· <?php echo e($review->created_at->diffForHumans()); ?></span>
                        </div>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
        ...
        <div class="mt-10 md:mt-24 pt-10 md:pt-12 border-t border-border">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                <?php echo $__env->make('shop.partials.product-card', ['product' => $rp], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
        <?php endif; ?>
...
<div x-data="{ showSticky: false }" x-init="window.addEventListener('scroll', () => { showSticky = window.scrollY > 500 })" class="fixed bottom-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-lg border-t border-border p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] lg:hidden transition-all duration-300" :class="showSticky ? 'translate-y-0' : 'translate-y-full'">
    <a href="<?php echo e(route('order.create', ['product_id' => $product->id])); ?>" class="flex items-center justify-center gap-2 w-full bg-[#c41e3a] text-white py-3.5 rounded-xl text-[11px] tracking-[0.3em] uppercase font-semibold shadow-[0_0_20px_rgba(196,30,58,0.25)] active:scale-[0.98] transition-all duration-300">
        Order Now — $<?php echo e(number_format($product->price, 0)); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\OPTIMOUS\OneDrive\Documents\yamagata\yamagata-oni\resources\views\shop\product.blade.php ENDPATH**/ ?>